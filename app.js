...
Caring with Compassion. Living with Dignity.`;
  ...
  function medicationTimeSelector(label,value,onChange,required=false){
    ...
    function toggle(time){const next=selected.includes(time)?selected.filter(x=>x!==time):[...selected,time].sort();onChange(next.join(', '))}
    return h('div',{className:'field medication-time-field'},h('label',null,label),
      h('details',{className:'time-dropdown'},h('summary',null,selected.length?selected.map(medicationTimeLabel).join(' • '):'Select time'),
        h('div',{className:'time-dropdown-menu'},MEDICATION_TIME_OPTIONS.map(opt=>h('label',{className:'time-option',key:opt.value},h('input',{type:'checkbox',checked:selected.includes(opt.value),onChange:()=>toggle(opt.value)}),h('span',null,opt.label))))),
