(() => {
  ...
  function Employees({profile}){
    ...
    const personnelFields=(state,setter,includeLogin=true)=>h(React.Fragment,null,
      field('Employee Name','full_name',state,setter,true),field('Employee ID (auto-generated if blank)','employee_id',state,setter,false),field('Designation','designation',state,setter,false),selectField('Role','role',state,setter,ROLES),
  ...
  function Patients(){
    ...
    async function printPatientIdCard(row){
      const url=await resolvePatientPhoto(row);const win=window.open('','_blank','width=760,height=700');if(!win){alert('Please allow pop-ups to print the Patient ID card.');return}
      win.document.write(`<!doctype html><html><head><title>Patient ID Card</title><style>body{font-family:Arial;margin:0;padding:30px;background:#eef6f4}.card{width:360px;height:570px;margin:auto;background:white;border-radius:24px;overflow:hidden;box-shadow:0 12px 35px #0002;border:2px solid #086b58}.head{background:#086b58;color:white;text-align:center;padding:22px}.head h1{margin:0;font-size:25px}.head p{margin:6px 0 0}.photo{width:130px;height:150px;border:4px solid white;border-radius:16px;object-fit:cover;background:#ddd;margin:-4px auto 16px;display:block;box-shadow:0 4px 15px #0003}.body{padding:16px 28px;text-align:center}.name{font-size:25px;font-weight:bold;color:#063f36}.role{font-size:18px;color:#086b58;margin:5px}.grid{text-align:left;margin-top:18px;line-height:1.75}.label{font-weight:bold;color:#555}.barcode{margin-top:15px;padding:10px;border-top:1px dashed #aaa;font-family:monospace}.print{display:block;margin:20px auto;padding:12px 24px}@media print{.print{display:none}body{background:white;padding:0}}</style></head><body><div class="card"><div class="head"><h1>SAMARA HEALTH CARE LLP</h1><p>Assisted Living Patient ID</p></div><div class="body">${url?`<img class="photo" src="${url}">`:`<div class="photo" style="display:flex;align-items:center;justify-content:center;font-size:48px">SC</div>`}<div class="name">${escapeHtml(row.full_name)}</div><div class="role">${escapeHtml(row.patient_category||'Patient')}</div><div class="grid"><div><span class="label">Patient ID:</span> ${escapeHtml(row.patient_id||'—')}</div><div><span class="label">Room / Bed:</span> ${escapeHtml(`${row.room_no||'—'} / ${row.bed_no||'—'}`)}</div><div><span class="label">Gender / Age:</span> ${escapeHtml(`${row.gender||'—'} / ${row.age||'—'}`)}</div><div><span class="label">Mobile:</span> ${escapeHtml(row.mobile||row.attendant_phone||'—')}</div><div><span class="label">Admission:</span> ${escapeHtml(row.admission_date||'—')}</div><div><span class="label">Special Nurse:</span> ${escapeHtml(row.special_nurse_required?(row.special_nurse_name||'Required'):'No')}</div></div><div class="barcode">${escapeHtml(row.patient_id||row.id)}</div></div></div><button class="print" onclick="window.print()">Print Patient ID Card</button></body></html>`);win.document.close();
    }
    ...
    return h(React.Fragment,null,
      ...
      selected&&details&&h('div',{className:'modal-backdrop'},h('div',{className:'card modal patient-master-modal'},
        ...
        h('div',{className:'patient-tab-content'},
          ...
          tab==='Vitals'&&h('div',{className:'section-card'},h('h4',null,'Vital Signs History'),details.vitals.length?details.vitals.map(v=>h('div',{className:'timeline-item',key:v.id},h('strong',null,`${fmt(v.recorded_at)} · BP ${v.systolic||'—'}/${v.diastolic||'—'}`),h('span',null,`Pulse ${v.pulse||'—'} · SpO₂ ${v.spo2||'—'} · Temp ${v.temperature||'—'} · Sugar ${v.blood_sugar||'—'} · ${v.alert_level||'Normal'}`))):sectionEmpty('No vital signs recorded.')),
  ...
  function RoomsBeds(){
    ...
    return h(React.Fragment,null,
      h(Section,{title:'Rooms & Beds',subtitle:'25-bed live occupancy linked to Patient Master'},h('div',{className:'bed-grid'},beds.map(n=>{const room=Math.ceil(n/2),bed=n%2?'A':'B';const p=patients.find(x=>String(x.room_no)===String(room)&&String(x.bed_no||'').toUpperCase()===bed);return h('div',{className:`bed-card ${p?'occupied':''}`,key:n},h('strong',null,`Room ${room} · Bed ${bed}`),h('span',null,p?`${p.patient_id||''} · ${p.full_name}`:'Available'),p&&p.special_nurse_required&&h('small',{className:'special-alert'},'Special nurse'))}))),
      unassigned.length?h(Section,{title:'Active Patients Awaiting Room / Bed',subtitle:'These Patient Master records are active but not yet assigned'},h('div',{className:'unassigned-patient-list'},unassigned.map(p=>h('div',{className:'timeline-item',key:p.id},h('strong',null,`${p.patient_id||'—'} · ${p.full_name}`),h('span',null,`${p.admission_type||'Admission'} · ${p.patient_category||'—'}`))))):null
    );
  }
