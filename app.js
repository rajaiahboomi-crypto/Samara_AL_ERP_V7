...
Caring with Compassion. Living with Dignity.`;
  ...
  function Admissions({profile}){
    ...
    async function submit(e){
      ...
      try{
        ...
        const medRows=meds.map(m=>{const start=m.start_date||new Date().toISOString().slice(0,10);const durationDays=m.duration==='Custom'?Number(m.custom_duration_days||0):({'Single Dose':0,'1 Day':1,'3 Days':3,'5 Days':5,'7 Days':7,'10 Days':10,'14 Days':14,'21 Days':21,'30 Days':30}[m.duration]??null);let endDate=null;if(durationDays!==null){const d=new Date(`${start}T00:00:00`);d.setDate(d.getDate()+Math.max(durationDays-1,0));endDate=d.toISOString().slice(0,10)}return {patient_id:patient.id,medicine_name:m.medicine_name,strength:m.strength,dose:m.dose,route:m.route,food_instruction:m.food_instruction,special_instruction:m.special_instruction,scheduled_times:m.times.split(',').map(x=>x.trim()).filter(Boolean),frequency:m.frequency,duration:m.duration,duration_days:m.duration==='Custom'?Number(m.custom_duration_days||0):durationDays,start_date:start,end_date:endDate,entered_by:user.id,verified_by:user.id}});
    ...
    return h('form',{className:'card panel',onSubmit:submit},
      ...
      h('div',{className:'section-card'},h('div',{className:'section-title'},h('h4',null,'3. Current medicines and prescription verification'),h('button',{type:'button',className:'btn btn-secondary',onClick:()=>setMeds([...meds,blankMedicine()])},'Add medicine')),meds.map((m,i)=>h('div',{className:'repeat-row',key:i},miniInput('Medicine',m.medicine_name,v=>updateRow(setMeds,meds,i,'medicine_name',v),true),miniInput('Strength',m.strength,v=>updateRow(setMeds,meds,i,'strength',v)),miniInput('Dose',m.dose,v=>updateRow(setMeds,meds,i,'dose',v),true),miniSelect('Route',m.route,['Oral','Injection','Topical','Inhalation','Drops','Other'],v=>updateRow(setMeds,meds,i,'route',v)),miniSelect('Food',m.food_instruction,['Before food','After food','With food','No restriction'],v=>updateRow(setMeds,meds,i,'food_instruction',v)),miniInput('Times',m.times,v=>updateRow(setMeds,meds,i,'times',v),true),miniSelect('Frequency',m.frequency,['Once Daily (OD)','Twice Daily (BD)','Three Times Daily (TDS)','Four Times Daily (QID)','Every 6 Hours','Every 8 Hours','Every 12 Hours','SOS / PRN','Weekly','Monthly'],v=>updateRow(setMeds,meds,i,'frequency',v)),miniSelect('Duration',m.duration,['Single Dose','1 Day','3 Days','5 Days','7 Days','10 Days','14 Days','21 Days','30 Days','Until Doctor Review','Long Term','Custom'],v=>updateRow(setMeds,meds,i,'duration',v)),m.duration==='Custom'&&miniInput('Custom days',m.custom_duration_days,v=>updateRow(setMeds,meds,i,'custom_duration_days',v),true,'number'),miniInput('Start date',m.start_date,v=>updateRow(setMeds,meds,i,'start_date',v),true,'date'),h('button',{type:'button',className:'icon-btn',onClick:()=>setMeds(meds.filter((_,n)=>n!==i)),disabled:meds.length===1},'Remove'),miniInput('Special instruction',m.special_instruction,v=>updateRow(setMeds,meds,i,'special_instruction',v))))),
  ...
  function Medicines(){
    ...
    return h(LogTable,{title:'Active Prescription & MAR',subtitle:'All medicines transcribed at admission',heads:['Patient','Medicine','Dose / Route','Frequency','Duration','Times','Food','Special instruction'],rows:rows.map(r=>[`${r.patients?.full_name} · ${r.patients?.room_no}-${r.patients?.bed_no}`,`${r.medicine_name} ${r.strength||''}`,`${r.dose} · ${r.route}`,r.frequency||'—',r.duration||'—',(r.scheduled_times||[]).map(String).join(', '),r.food_instruction||'—',r.special_instruction||'—'])})
  }
