(() => {
  ...
  function Patients(){
    ...
    async function resolvePatientPhoto(p){
      ...
      if(!path)return '';
      const {data}=await client.storage.from('patient-documents').createSignedUrl(path,600);
      return data?.signedUrl||'';
    }
    async function openPatient(p){
      setSelected(p);setPhotoUrl('');
      const [m,c,ph,d,url]=await Promise.all([
        client.from('medication_orders').select('*').eq('patient_id',p.id),
        client.from('care_orders').select('*').eq('patient_id',p.id),
        client.from('physiotherapy_orders').select('*').eq('patient_id',p.id),
        client.from('patient_documents').select('*').eq('patient_id',p.id).order('created_at',{ascending:false}),
        resolvePatientPhoto(p)
      ]);
      setDetails({meds:m.data||[],care:c.data||[],physio:ph.data||[],docs:d.data||[]});setPhotoUrl(url);
    }
    ...
    async function printPatientIdCard(row){
      const url=await resolvePatientPhoto(row);
      const win=window.open('','_blank','width=760,height=700');if(!win){alert('Please allow pop-ups to print the Patient ID card.');return}
